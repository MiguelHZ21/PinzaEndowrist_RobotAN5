...
public class ControlArticular : MonoBehaviour
{
    ...
    private void AddJointPosition()
    {
        ...
        {
            endoWristPositionsList.Add(new float[4]); // Valores por defecto si no está asignado
        }
    ...
    private void UpdateCoordinatesDisplay()
    {
        ...
        for (int i = 0; i < jointPositionsList.Count; i++)
        {
            ...
            {
                float[] ewp = endoWristPositionsList[i];
                coordinatesDisplay.text += $", Pinza: [{ewp[0]:F2}, {ewp[1]:F2}, {ewp[2]:F2}, {ewp[3]:F2}]";
            }
