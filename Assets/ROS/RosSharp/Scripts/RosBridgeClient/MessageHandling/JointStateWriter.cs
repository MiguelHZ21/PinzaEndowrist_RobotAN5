...
public class JointStateWriter : MonoBehaviour
{
    ...
    private IEnumerator InterpolateRotation()
    {
        ...
        switch (JointName)
        {
            ...
            case "j6_Link":
                ...
                break;
            // --- Pinza (EndoWrist) ---
            case "Endo_eje": // shaft -> rota en eje X
            case "shaft":
                targetRotation.x = targetValue;
                targetRotation.y = initialRotation.y;
                targetRotation.z = initialRotation.z;
                break;
            case "Endo_muneca": // wrist -> rota en eje Z
            case "wrist":
                targetRotation.x = initialRotation.x;
                targetRotation.y = initialRotation.y;
        ...
        // Pinza: mantener ejes fijos según la articulación
        else if (JointName == "Endo_eje" || JointName == "shaft")
        {
    ...
    private float GetCurrentAngleFromTransform()
    {
        ...
        switch (JointName)
        {
            ...
            case "j6_Link":
                return rot.x;
            // Eje (shaft) del EndoWrist rota en X
            case "Endo_eje":
            case "shaft":
                return rot.x;
            // Muñeca y mandíbulas del EndoWrist rotan en Z
            case "Endo_muneca":
            case "wrist":
    ...
    // Devuelve el ángulo actual del transform en Unity, normalizado [-180, 180]
    // Lee directamente del transform del GameObject, sin depender de UrdfJoint
    // Para articulaciones con HingeJoint (como las mandíbulas), lee de hingeJoint.angle
    public float GetCurrentUnityAngle()
    {
        // Si tiene HingeJoint, leer el ángulo desde ahí (mandíbulas de la pinza)
        HingeJoint hinge = GetComponent<HingeJoint>();
        if (hinge != null)
        {
            return NormalizeAngle(hinge.angle);
        }

        // Si no tiene HingeJoint, leer del transform directamente
